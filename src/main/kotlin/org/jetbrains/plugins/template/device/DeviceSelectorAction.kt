...
/**
 * Toolbar action that displays a dropdown selector for HarmonyOS devices.
 */
class DeviceSelectorAction : AnAction(), CustomComponentAction, DumbAware {
    ...
    override fun createCustomComponent(presentation: Presentation, place: String): JComponent {
        ...
        // Set foreground color to adapt to toolbar theme
        ...
        // Forward mouse events from labels to button
        ...
        // 延迟更新初始状态，确保 DeviceService 已经初始化
        ApplicationManager.getApplication().invokeLater {
            ...
            if (project != null && !project.isDisposed) {
                ...
                // 将 timer 存储到 button 中，以便在需要时可以停止
    ...
    override fun update(e: AnActionEvent) {
        ...
        // Register listener on first update
        ...
        // 从 ExecutionTargetManager 获取当前活动的 target
        ...
        val displayDevice: HarmonyDevice? = when {
            activeTarget is HarmonyExecutionTarget -> {
                // 使用 ExecutionTarget 选中的设备
                activeTarget.getDevice()
            }
            else -> {
                // 回退到 DeviceService 的选中设备
                deviceService.getSelectedDevice()
            }
        }
    ...
    private fun showDevicePopup(dataContext: DataContext, component: Component?) {
        ...
        if (devices.isEmpty()) {
            // Show a helpful message when no devices are connected
    ...
    private fun queueUpdate(project: Project, presentation: Presentation) {
        println("=== DeviceSelectorAction.queueUpdate() CALLED ===")
        println("Project: ${project.name}")
        ApplicationManager.getApplication().invokeLater {
            if (!project.isDisposed) {
                println("Incrementing ActivityTracker to trigger UI update")
                ...
                if (button != null) {
                    ...
                    if (iconLabel != null && textLabel != null) {
                        ...
                        button.revalidate()
                        button.repaint()
                        println("UI updated: icon=$icon, text=$text")
                    }
                }
            } else {
                println("Project disposed, skipping update")
            }
        }
        println("=== DeviceSelectorAction.queueUpdate() COMPLETE ===")
    }
    ...
    /**
     * Action for selecting a specific device.
     */
    private class SelectDeviceAction(
        ...
        override fun actionPerformed(e: AnActionEvent) {
            println("=== SelectDeviceAction.actionPerformed() ===")
            println("Selected device: ${device.displayName}")
            
            // 更新 DeviceService
            DeviceService.getInstance(project).setSelectedDevice(device)
            
            // 更新 ExecutionTargetManager
            val targetManager = com.intellij.execution.ExecutionTargetManager.getInstance(project)
            val runManager = com.intellij.execution.RunManager.getInstance(project)
            ...
            if (selectedConfig != null) {
                ...
                if (harmonyTarget != null) {
                    println("Setting active target to: ${harmonyTarget.displayName}")
                    targetManager.activeTarget = harmonyTarget
                    println("Active target is now: ${targetManager.activeTarget.displayName}")
                } else {
                    println("Could not find HarmonyExecutionTarget for device: ${device.deviceId}")
                }
            }
        }
